...
with mock.patch(
    ...
    from src.app.flask_postgresql.controllers.items.\
        deactivate_item_controller import DeactivateItemController


def test_deactivate_item_controller(
        monkeypatch,
        mocker,
...
deactivate_item_controller.ItemPostgreSQLRepository"
    )
    mocker_presenter = mocker.patch(
        "src.app.flask_postgresql.controllers.items.\
deactivate_item_controller.DeactivateItemPresenter"
    )
    mock_use_case = mocker.patch(
        "src.app.flask_postgresql.controllers.items.\
deactivate_item_controller.DeactivateItemUseCase"
    )
    ...
    controller = DeactivateItemController(logger_mock)
